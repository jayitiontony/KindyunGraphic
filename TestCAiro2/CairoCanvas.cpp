...
void CairoCanvas::SwapBuffers() {
    ...
    // ----------------------------------------------------------------------
    // 把 draw buffer 的像素整体复制到 read buffer。
    // 单线程 MFC (timer 跟 OnPaint 都在 UI 线程) 下, 其实可以让 draw ==
    // read 直接读写同一份 buffer; 但为了保持 KG_GetData() 拿到的 read 指针
    // 始终是"已完成帧", 仍然做一次 memcpy, 简单且不撕裂。
    //
    // ★ 关键: 这里 **不再** cairo_destroy + cairo_create 重建 cairo_t。
    //   旧实现每帧一次 destroy/create, 会在 cairo 的 freed_pool /
    //   scaled_font_map / toy_font_face 缓存里持续累积泄漏对象, 配合
    //   crtdbg 验证时直接刷出几十条 leak 报告。
    // ----------------------------------------------------------------------

    if (m_surfaceDraw == m_surfaceRead) {
        // 同源 surface (单 buffer 模式), 不用拷贝, 直接 flush 就够。
        cairo_surface_flush(m_surfaceDraw);
        return;
    }

    // 1) 把 cairo 内部缓存写回 draw 绑定的 buffer (m_bufA)
    cairo_surface_flush(m_surfaceDraw);

    // 2) memcpy: m_bufA -> m_bufB
    const int   height      = m_height;
    const int   bytesPerRow = m_stride;
    const unsigned char* src = cairo_image_surface_get_data(m_surfaceDraw);
    unsigned char*       dst = cairo_image_surface_get_data(m_surfaceRead);
    if (src && dst) {
    ...
    // 3) 告诉 cairo read buffer 内容已变, 下次外部读时不要走 cache
    cairo_surface_mark_dirty(m_surfaceRead);

    // 注意: cairo_t (m_cr) 一直绑 m_surfaceDraw, 不动; 下次绘制直接
    // 往同一个 surface 写, cairo 自己会重画, 不需要 clear / recreate。
}
